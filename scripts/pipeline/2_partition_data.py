...
def main() -> None:
    """
    Partitions the data into folds for cross-validation.
    """
    ...
    print(
        f"specimens that will not be used due to missing labels: {unmatched}"
    )
    ...
    for indices in groups.groups.values():
        ...
        # fold will always have less than the other folds if
        # len(group_patients) is not divisible by NUM_FOLDS
